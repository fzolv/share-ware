...
- We are using h2 database for ease of testing and demo purpose. Please use http://localhost:8080/shareware/h2-console
  to access the database console
- For obtaining jwt for running shareware apis. Visit http://localhost:8080/shareware/oauth2/authorization/google to
...
Share-Ware lets users create groups, add expenses, settle balances, and compute who owes whom. It provides JWT-based
authentication, fine-grained resource authorization, event streaming for integrations, and pluggable locking and
messaging backends.
...
- In case in future, I need to add Multiple currency support, I can add a new Splitting Strategy that will normalize the currency and then split
...
- e.g. I can add a event push message in case a expense is updated so user can refresh and check new balance before settlement
...
- Resource-based authorization:
    - `ResourceAuthorizationService` resolves effective role per resource type (USER, GROUP, EXPENSE, BALANCE,
      SETTLEMENT)
...
- Like user should receive notification when he is added to a group or when an expense is created in a group he is part
  of, etc
